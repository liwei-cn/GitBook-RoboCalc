...
_A robotic platform also defines \(represented by a symbol **i**_ in the diagram_\) interfaces that group events. Note that the operations and events can not be grouped in the same interface. The events are connected between different components to exchange message._ The implementation of a robotic platform must extend Interface subclasses.
