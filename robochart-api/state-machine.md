...
In the obstacle avoidance example, the transition `t1` has a trigger of the form`obstacle?dir`and also reset the clock `T`. This transition is implemented as the classt`1`, where`Reg`is implemented as a function that calls the function`Reg`of the channel`obstacle`of the state machine with source name `StmMovement` \(identifying the state machine StmMovement\) and undefined parameter`Optional<Loc>()`. The result of the`Reg`function of the channel is a pointer to an event that is recorded in the variable`event`.
